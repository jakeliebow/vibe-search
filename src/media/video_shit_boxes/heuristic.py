    ...
    for yolo_id in yolo_objects:
        ...
        for index,detection in enumerate(yolo_object_track.detections):
            ...
            if face:
                mar=face.mar
                if mar:
                    mar_derivative=(mar - last_mar.mar)/(max(1,index - last_mar.index))
                    face.mar_derivative = mar_derivative
                    last_mar=MarAtIndex(index,mar_derivative)
...
def select_diverse_fast(
    ...
    *,
    ...
    N = E.shape[0]
    if N == 0 or k <= 0:
        return {"indices": [], "assign": np.array([]), "assign_dist": np.array([]), "coverage": 0.0}
    ...
    if simhash_bits and simhash_bits > 0:
        ...
        # unique rows
        _, unique_idx = np.unique(packed.view([('', packed.dtype)] * packed.shape[1]), return_index=True)
        unique_idx.sort()
    ...
    mean = E_core.mean(axis=0); mean /= (np.linalg.norm(mean) + 1e-9)
