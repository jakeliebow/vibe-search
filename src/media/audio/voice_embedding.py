...
def generate_voice_embedding(audio_array: np.ndarray) -> np.ndarray:
    ...
    if audio_tensor.shape[-1] < min_samples:
        ...
        audio_tensor = torch.nn.functional.pad(
            audio_tensor, (0, padding_needed), mode="constant", value=0
        )
...
def compute_voice_embeddings_per_speaker(
    diarized_audio_segments_by_speaker_track_index: Dict[str, SpeakerTrack],
):
    for (
        speaker_label,
        speaker_track,
    ) in diarized_audio_segments_by_speaker_track_index.items():
        ...
        if speaker_track.segments and all(
            s.audio_array is not None for s in speaker_track.segments
        ):
            concatenated_audio_array = np.concatenate(
                [segment.audio_array for segment in speaker_track.segments]
            )
            speaker_track.voice_embedding = generate_voice_embedding(
                concatenated_audio_array
            )
            speaker_track.audio_data = concatenated_audio_array
        else:
